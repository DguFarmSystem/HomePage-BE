...
public class InventoryService {
    ...
    private List<Inventory> addToInventory(Player player, Goods goods, int count) {
    ...
    private void deleteInventory(Player player, Long storeGoodsNumber, int count) {
        for (int i = 0; i < count; i++) {
            Inventory oldest = inventoryRepository
                    .findFirstByPlayerAndGoodsType_GoodsNumberOrderByInventoryIdAsc(player, storeGoodsNumber)
    ...
    private int computeDeltaCount(Player player, Long storeGoodsNumber, int newCount) {
        int current = inventoryRepository.countByPlayerAndGoodsType_GoodsNumber(player, storeGoodsNumber);
    ...
    public List<InventoryResponseDTO> getInventory(Long userId){
        ...
        return inventoryRepository.countInventoryGroupByGoodsType(player);
    }
    ...
    public List<InventoryResponseDTO> updateInventoryObject(Long userId, InventoryUpdateRequestDTO requestDTO){
        ...
        //행 추가 또는 삭제
        if (delta > 0) {
            List<Inventory> temp = addToInventory(player, goods, delta);
